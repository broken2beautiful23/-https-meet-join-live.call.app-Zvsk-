  ...
  return (
    <div className="flex-1 flex flex-col bg-[#202124] overflow-hidden relative">
      <div className="flex-1 flex overflow-hidden">
            ...
            <div className={`relative bg-[#3c4043] rounded-xl overflow-hidden transition-all duration-300 ${isAiTalking ? 'ring-2 ring-[#8ab4f8]' : ''}`}>
               <div className="absolute inset-0 flex items-center justify-center">
                  <div className="relative">
                     <img src={`https://picsum.photos/seed/${host.seed}/200`} className="w-32 h-32 md:w-48 md:h-48 rounded-full object-cover shadow-2xl border-4 border-[#202124]" alt={host.name} />
                     {isAiTalking && (
        ...
        {showChat && (
          <ChatSidebar messages={messages} onSendMessage={handleSendMessage} onClose={() => setShowChat(false)} />
        )}
      </div>
      ...
      <div className="h-20 bg-[#202124] flex items-center justify-between px-6 z-20">
        <div className="hidden lg:flex items-center gap-4 text-white font-medium">
           ...
           <span className="text-[#e8eaed]">emw-call43</span>
        </div>
        <div className="flex items-center gap-3 mx-auto lg:mx-0">
           <button onClick={toggleMic} className={`p-3 rounded-full transition-all border ${isMicOn ? 'bg-[#3c4043] border-[#5f6368] hover:bg-[#4a4d51]' : 'bg-[#ea4335] border-[#ea4335] hover:bg-[#d93025]'}`}>
              {isMicOn ? (
           ...
           <button onClick={toggleCamera} className={`p-3 rounded-full transition-all border ${isCameraOn ? 'bg-[#3c4043] border-[#5f6368] hover:bg-[#4a4d51]' : 'bg-[#ea4335] border-[#ea4335] hover:bg-[#d93025]'}`}>
              {isCameraOn ? (
           ...
           <button onClick={onLeave} className="px-6 py-3 bg-[#ea4335] hover:bg-[#d93025] rounded-full flex items-center gap-2 transition-colors shadow-lg">
        ...
        <div className="hidden lg:flex items-center gap-2">
           <button onClick={() => setShowChat(!showChat)} className={`p-3 rounded-full transition-colors ${showChat ? 'bg-[#8ab4f8] text-[#202124]' : 'text-white hover:bg-[#3c4043]'}`}>
